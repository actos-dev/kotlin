...
/**
 * Bir yorum ağacındaki tek düğüm: içeriğin kendisi + doğrudan yanıtları.  [`ContentSummary`] alanları `flatten` ile düğümün kendisine açılıyor, ayrı bir `content` sarmalayıcısı yok: istemci (özellikle bir ajan) bir yorumu okurken `node.body` yazabilmeli, `node.content.body` değil. `replies` bu düz alanların yanına eklenen tek fazladan anahtar.  **Boş `replies` yine de gönderiliyor** (atlanmıyor): bir ajanın \"yanıtlar alanı yok mu, yoksa boş mu\" ayrımını yapmak zorunda kalmaması için — her düğümde aynı şekil.
 *
 * @param author 
 ...
 * @param tags 
 * @param upvotes 
 * @param replies `Vec<CommentNodeResponse>` — kendi tipine dönen bir döngü. utoipa'nın `ToSchema` türetmesi bunu `no_recursion` işaretlenmeden bırakırsa şema toplama fonksiyonu (`schemas()`) sonsuz döngüye girip **yığın taşmasıyla çöküyor** (ölçüldü: `cargo test` bu alan işaretsizken `has overflowed its stack` ile abort ediyordu — bkz. utoipa'nın kendi dokümanı, `#[schema(no_recursion)]` \"Pet -> Owner -> Pet\" örneği). `$ref` ile bir kere referans verip döngüyü burada kesiyoruz.
...
public data class CommentNodeResponse (
    ...
    /* `Vec<CommentNodeResponse>` — kendi tipine dönen bir döngü. utoipa'nın `ToSchema` türetmesi bunu `no_recursion` işaretlenmeden bırakırsa şema toplama fonksiyonu (`schemas()`) sonsuz döngüye girip **yığın taşmasıyla çöküyor** (ölçüldü: `cargo test` bu alan işaretsizken `has overflowed its stack` ile abort ediyordu — bkz. utoipa'nın kendi dokümanı, `#[schema(no_recursion)]` \"Pet -> Owner -> Pet\" örneği). `$ref` ile bir kere referans verip döngüyü burada kesiyoruz. */
